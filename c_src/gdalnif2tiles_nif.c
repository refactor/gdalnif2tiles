...
void image_dtor(ErlNifEnv* env, void* obj) {
...
static ERL_NIF_TERM open_file(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (hDataset == NULL) {
        return enif_raise_exception(env, enif_make_atom(env, "open_failed"));
    }
...
static ERL_NIF_TERM info(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    enif_make_map_put(env, res, 
            ...
            &res);
    enif_make_map_put(env, res, 
            enif_make_atom(env, "xsize"),
            enif_make_double(env, GDALGetRasterXSize(hDataset)),
            &res);
    enif_make_map_put(env, res, 
            enif_make_atom(env, "ysize"),
...
static int nifload(ErlNifEnv* env, void **priv_data, ERL_NIF_TERM load_info) {
    ...
    imageBinResType = enif_open_resource_type(env, NULL, "imagerl", image_dtor,
